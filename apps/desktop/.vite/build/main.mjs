...
class AxiosTransformStream extends stream.Transform {
  ...
  _transform(chunk, encoding, callback) {
    ...
    const transformChunk = (_chunk, _callback) => {
      ...
      if (maxRate) {
        const now = Date.now();
        if (!internals2.ts || (passed = now - internals2.ts) >= timeWindow) {
          internals2.ts = now;
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (isHttp2) {
      ...
      if (config.transport) {
        ...
        transport = isHttpsRequest ? https : require$$3;
      } else {
...
class ValorantLocalAPI {
  ...
  initialize(lockfileData) {
    ...
    this.client = axios.create({
      ...
      httpsAgent: new https.Agent({
        ...
        // Riot Client uses self-signed certificate
      }),
      auth: {
        username: "riot",
   ...
   */
  async getAuthTokens() {
    const response = await this.request("/entitlements/v1/token");
    return {
  ...
  async getClientVersion() {
    const response = await this.request("/product-session/v1/external-sessions");
   ...
   */
  async getCurrentPartyId(puuid) {
    try {
      const response = await this.request("/chat/v4/presences");
   ...
   */
  async getPregameInfo(puuid) {
    try {
      const response = await this.request(
        `/pregame/v1/players/${puuid}`
      );
      if (response.ID) {
        return await this.request(`/pregame/v1/matches/${response.ID}`);
      }
      return null;
    } catch (error) {
    ...
    try {
      ...
      if (playerPresence?.private) {
        ...
        if (privateData.partyId) {
          return await this.request(`/parties/v1/parties/${privateData.partyId}`);
        }
      }
      return null;
    } catch (error) {
...
class ValorantRemoteAPI {
   ...
   */
  initialize(authTokens, clientVersion, region, shard) {
   ...
   */
  async generatePartyCode(partyId) {
    if (!this.client) {
   ...
   */
  async joinPartyByCode(code) {
    if (!this.client) {
      return { success: false, error: "API not initialized" };
    }
    try {
      const response = await this.client.post(`/parties/v1/players/joinbycode/${code}`);
      return {
        success: true,
   ...
   */
  async setPartyAccessibility(partyId, open) {
    if (!this.client) {
      return false;
    }
    try {
...
class LockfileWatcher extends EventEmitter {
   ...
   */
  hasLockfileChanged(newData) {
    if (!this.currentLockfileData) return true;
    return this.currentLockfileData.pid !== newData.pid || this.currentLockfileData.port !== newData.port || this.currentLockfileData.password !== newData.password;
  }
  /**
...
class RealtimeChannel {
  ...
  _trigger(type2, payload, ref2) {
    ...
    if (["insert", "update", "delete"].includes(typeLower)) {
      ...
      }).map((bind2) => bind2.callback(handledPayload, ref2));
    } else {
      (_b = this.bindings[typeLower]) === null || _b === void 0 ? void 0 : _b.filter((bind2) => {
        ...
        bind2.callback(handledPayload, ref2);
      });
    }
  }
  /** @internal */
  ...
  _off(type2, filter2) {
    ...
    if (this.bindings[typeLower]) {
      this.bindings[typeLower] = this.bindings[typeLower].filter((bind2) => {
        var _a2;
        return !(((_a2 = bind2.type) === null || _a2 === void 0 ? void 0 : _a2.toLocaleLowerCase()) === typeLower && RealtimeChannel.isEqual(bind2.filter, filter2));
      });
    }
    return this;
  }
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (typeof Blob !== "undefined" && fileBody instanceof Blob) {
          ...
          const isStream2 = typeof ReadableStream !== "undefined" && body instanceof ReadableStream || body && typeof body === "object" && "pipe" in body && typeof body.pipe === "function";
          if (isStream2 && !options.duplex) {
            options.duplex = "half";
          }
        }
...
function createSiweMessage(parameters) {
  var _a2;
  const { chainId, domain, expirationTime, issuedAt = /* @__PURE__ */ new Date(), nonce, notBefore, requestId, resources, scheme, uri: uri2, version: version2 } = parameters;
  {
...
class HeartbeatManager {
  ...
  lastStatus = null;
  onStatusChange;
  constructor(localApi2, lockfileWatcher2) {
    this.localApi = localApi2;
    this.lockfileWatcher = lockfileWatcher2;
  }
  /**
  ...
  async collectHeartbeatData() {
    ...
    let valorantStatus = "not_running";
    let partyId = null;
    let partyCode = null;
    let partySize = 0;
    let playerPuuid = null;
  ...
  hasStatusChanged(newStatus) {
    ...
    return this.lastStatus.isOnline !== newStatus.isOnline || this.lastStatus.valorantRunning !== newStatus.valorantRunning || this.lastStatus.valorantStatus !== newStatus.valorantStatus;
  }
  /**
...
class LFGService {
  ...
  constructor(localApi2, remoteApi2, lockfileWatcher2) {
    this.localApi = localApi2;
    this.remoteApi = remoteApi2;
    this.lockfileWatcher = lockfileWatcher2;
  }
  /**
   ...
   * Invite player to party by Riot ID
   */
  async inviteToParty(gameName, tagLine) {
    if (!this.isInitialized) {
      const initialized = await this.initialize();
      if (!initialized) {
        return { success: false, error: "Valorant not running or not initialized" };
   ...
   */
  mapPartyToLFGInfo(party, currentPuuid) {
...
class CommandListener {
  ...
  userId = null;
  isListening = false;
  constructor(lfgService2) {
    this.lfgService = lfgService2;
  }
  /**
  ...
  async handleGenerateCode() {
    ...
    return {
      ...
      success: result.success,
      data: result.success ? { code: result.code } : void 0,
      error: result.error
    };
  }
  /**
   ...
   */
  async handleInvitePlayer(gameName, tagLine) {
...
let lfgService;
let commandListener;
let currentStatus = "not_running";
let mainWindow = null;
let supabaseUrl = "";
function setupIpcHandlers(window2) {
  mainWindow = window2;
  ...
  ipcMain.handle(IPC_CHANNELS.VALORANT_GET_STATUS, async () => {
  ...
  ipcMain.handle(IPC_CHANNELS.LFG_GET_PARTY_INFO, async () => {
  ...
  ipcMain.handle(IPC_CHANNELS.LFG_JOIN_PARTY_BY_CODE, async (_event, { code }) => {
