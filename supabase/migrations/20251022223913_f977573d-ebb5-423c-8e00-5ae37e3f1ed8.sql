-- Fix unique constraint issue for phantom teams
-- Make team names truly unique by adding timestamp

CREATE OR REPLACE FUNCTION public.rpc_fill_tournament(
  tournament_id_input uuid,
  desired_size integer DEFAULT NULL::integer
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Create unique suffix using milliseconds timestamp
  unique_suffix := EXTRACT(EPOCH FROM NOW())::TEXT || '_';
  ...
  FOR i IN 1..teams_needed LOOP
    -- Create phantom team with unique name
    INSERT INTO teams (
      owner_id,
      name,
      tag,
      ...
      current_user_id,
      'Фантомная команда ' || unique_suffix || i::TEXT,
      'PH' || i::TEXT,
      NULL,
      TRUE,
      phantom_source_val,
    ...
    FOR j IN 1..5 LOOP
      INSERT INTO profiles (
        ...
        'PhantomTeam' || unique_suffix || i::TEXT || 'P' || j::TEXT,
        'TEST',
        TRUE,
        phantom_source_val
