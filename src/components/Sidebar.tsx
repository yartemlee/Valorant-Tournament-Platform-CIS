import { Users, Trophy, Home, User, Swords, BarChart2, Search, Shield } from "lucide-react";
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
              onClick={(e) => item.inDevelopment && e.preventDefault()}
              className={({ isActive }) =>
                cn(
                  "flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-300 relative group",
                  item.inDevelopment ? "cursor-not-allowed opacity-70" : "hover:bg-secondary/80 hover:shadow-glow-primary",
            ...
            >
              ...
              {item.inDevelopment && (
                <Badge variant="outline" className="ml-auto text-[10px] px-1 py-0 h-5 border-muted-foreground/30 text-muted-foreground">
                  Скоро
                </Badge>
              )}
            </NavLink>
